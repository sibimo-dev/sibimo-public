...
function useCurrentLocation() {
  ...
  // Catatan: Geolocation API browser hanya berfungsi di HTTPS atau localhost.
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      ...
      try {
        ...
        // fallback: tetap tampilkan koordinat mentah kalau layanan alamat gagal
...
const files = ref([]) // { id, file, name, sizeLabel, previewUrl }
const fileError = ref('')
...
function onFileInputChange(e) {
  ...
  e.target.value = '' // supaya file yang sama bisa dipilih ulang kalau dihapus lalu ditambah lagi
}
...
async function submitReport() {
  ...
  // TODO(dev): ganti dengan pemanggilan API backend sesungguhnya, kirim juga
  // `files` (multipart/form-data) dan `locationCoords` kalau tersedia.
...
/* ---------------- Batal ----------------
   Diperbaiki: sebelumnya memakai router.back(), yang hanya berpindah ke
   halaman sebelumnya di history browser TANPA mereset state form.
   Efeknya: kategori/lokasi tetap tersimpan, dan halaman malah
   berpindah ke halaman lain (misal Profile) alih-alih tetap di form Aduan.
...
<template>
  <div class="mx-auto max-w-6xl px-4 py-8 md:px-6">
    <h1 class="text-2xl font-bold text-slate-900 md:text-3xl">Laporkan Aduan</h1>
    ...
    <div
      v-if="submitSuccess"
      class="mt-6 flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3.5 text-sm text-emerald-700"
    >
      <span class="mt-0.5 flex h-5 w-5 flex-none items-center justify-center rounded-full bg-emerald-500 text-xs text-white">
        ✓
      </span>
      <div>
    ...
    <div class="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      <form class="space-y-8 lg:col-span-2" @submit.prevent="submitReport">
        ...
        <section>
          <div class="flex items-center gap-2.5">
            <span class="flex h-6 w-6 items-center justify-center rounded-full bg-slate-800 text-xs font-bold text-white">1</span>
          ...
          <div class="mt-3 border-b border-slate-100 pb-6">
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <button
                ...
                class="relative flex flex-col items-center gap-2 rounded-xl border p-4 text-center transition-all duration-200"
                :class="
              ...
              >
                <span
                  ...
                  class="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500 text-[11px] text-white shadow"
                >
                  ✓
                </span>
        ...
        <section>
          <div class="flex items-center gap-2.5">
            <span class="flex h-6 w-6 items-center justify-center rounded-full bg-slate-800 text-xs font-bold text-white">2</span>
          ...
          <div class="mt-4 space-y-5 border-b border-slate-100 pb-6">
            <div>
              ...
              <input
                ...
                class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-100"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                class="w-full rounded-xl border px-4 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2"
                :class="
            ...
            <div>
              ...
              <div
                class="flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 focus-within:border-slate-400 focus-within:ring-2 focus-within:ring-slate-100"
              >
                ...
                <button
                  ...
                  class="flex shrink-0 items-center gap-1.5 whitespace-nowrap text-xs font-semibold text-blue-600 hover:text-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <span
        ...
        <section>
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2.5">
              <span class="flex h-6 w-6 items-center justify-center rounded-full bg-slate-800 text-xs font-bold text-white">3</span>
          ...
          <div class="mt-4">
            <div
              ...
              :class="isDragging ? 'border-blue-400 bg-blue-50' : 'border-slate-200 bg-slate-50 hover:bg-slate-100'"
              @click="openFilePicker"
              @dragover.prevent="isDragging = true"
              @dragleave.prevent="isDragging = false"
              @drop.prevent="onDrop"
            >
              <span class="flex h-11 w-11 items-center justify-center rounded-full bg-white text-xl shadow-sm">☁️</span>
            ...
            <ul v-if="files.length" class="mt-3 space-y-2">
              <li
                v-for="f in files"
                :key="f.id"
                class="flex items-center gap-3 rounded-xl border border-slate-100 bg-white px-3 py-2"
              >
                <img
                  v-if="f.previewUrl"
                  :src="f.previewUrl"
                ...
                />
                <span v-else class="flex h-10 w-10 flex-none items-center justify-center rounded-lg bg-slate-100 text-lg">🎞️</span>
                ...
                <button
                  ...
                  class="flex h-7 w-7 flex-none items-center justify-center rounded-full text-slate-400 hover:bg-slate-100 hover:text-red-500"
        ...
        <div class="flex flex-wrap items-center justify-end gap-3 pt-2">
          ...
          <button
            type="button"
            @click="saveDraft"
            class="rounded-xl bg-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-300"
          >
            Simpan Draf
          </button>
          <button
            ...
            class="flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40"
          >
      ...
      <!-- ===== SIDEBAR ===== -->
      <aside class="h-fit rounded-2xl border border-blue-100 bg-white p-6 shadow-md shadow-blue-100/40">
        <div class="flex items-center gap-2">
          <span class="h-px w-6 bg-blue-500" />
        ...
        <ul class="mt-5 space-y-4">
          <li class="flex gap-3 rounded-xl p-2 transition-colors hover:bg-slate-50">
          ...
          <li class="flex gap-3 rounded-xl p-2 transition-colors hover:bg-slate-50">
            <span class="mt-0.5 flex h-9 w-9 flex-none items-center justify-center rounded-full bg-blue-100 text-blue-600">
              <svg viewBox="0 0 20 20" fill="currentColor" class="h-4.5 w-4.5">
          ...
          <li class="flex gap-3 rounded-xl p-2 transition-colors hover:bg-slate-50">
            <span class="mt-0.5 flex h-9 w-9 flex-none items-center justify-center rounded-full bg-indigo-100 text-indigo-600">
              <svg viewBox="0 0 20 20" fill="currentColor" class="h-4.5 w-4.5">
